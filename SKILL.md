...
summarize 一日趋势时，"今日新动向"如果只写"AI Agent 全栈继续火热"这类无对比的空话，对用户毫无价值。**只有把今日上榜与近 3 日上榜集合做对比，才有可能识别出"谁登顶/让位"、"谁是真正的'新面孔'"、"谁连续霸榜"等真实信号**——这些信号需要历史归档才能观察到。本 skill 通过 ./github-trending-knowledge/ 目录累积历史归档，正是为了让这一步有数据可读。

**"新面孔仓库"严格定义**：今日首次出现在趋势榜、且**近 3 天（昨日 + 前日）均未上榜**的仓库——即"近 3 天未上榜、今日首次上榜"。**与名次无关**：今日哪怕某仓库仅排在第 18 名，只要满足"近 3 天未上榜"，也算新面孔。**必须连续两日未上榜才算新仓库**：仓库昨天上榜、今天未上榜、明天又上榜，**不算**新仓库。

**具体动作**：

1. 用 `Glob` 列出 `<project_root>/github-trending-knowledge/*/github-trending.md` 全部历史归档。
2. 用 `Read` 读取**最近 2 天**（即昨日 + 前日）的归档（按 `YYYY-MM-DD` 排序后取最新两份日期），从每份归档的 `# owner/repo` 标题顺序提取全部仓库 `full_name`，合并为一个去重集合。
3. 把今日即将抓取的整张榜单（N 个仓库）与昨日 Top 5 做对比，识别：
   - **易主**：昨日 #1 今日是否被取代（连续霸榜 N 天后让位 = 强信号）
   - **落榜**：昨日 Top 5 今日跌出 Top 5
   - **排名跃迁**：同一仓库在 Top 5 内排名大幅变化
4. **新面孔去重（必做）**：
   - 令 **S** = 昨日归档 ∪ 前日归档 中的全部仓库 `full_name` 集合（即"近 3 天已上榜仓库集合"，S 仅来自昨日 + 前日两份归档，今日尚未上榜，不在 S 内）。
   - 令 **T** = 今日全部上榜仓库 `full_name` 集合（不论名次，整张榜单 N 个仓库都算）。
   - **新面孔仓库集合 N** = T − S（即今日上榜但昨日 + 前日都没上榜的仓库；**运算方向必须是 T 减 S**，不能用 S 减 T，否则会得到"曾经上榜今日又出现的仓库"，与定义完全相反）。
   - **与名次无关**：N 不再做任何 Top 5 过滤——今日哪怕某仓库仅排在第 18 名，只要满足"近 3 天未上榜"，就属于 N。
   - summary.md "今日新动向"小节的描述对象：从 N 中按代表性挑 ≤3 个（**优先选落在 Top 5 的新面孔**，因为它们今日热度更高、信号更强；如 N 中无 Top 5 成员，则在 N 中按 stars_today 降序挑 ≤3 个）；若 N 整体 ≤3 个，则全部介绍；若 N ≥3 个但内部多仓库同属一个方向/领域，则合并概括该方向、不逐个介绍。
5. 把以上对比结果以 `### 跨日对比备忘` 形式记在 Step 0 末尾（仅本对话内参考，最后不要写进交付物），例如：
   ```
   ### 跨日对比备忘
   - 昨日 Top 5: [A, B, C, D, E]
   - 今日 Top 5: [X, A, C, F, G]
   ...
   - 落榜: B, D, E
   - 近 3 天已上榜集合 S: {A, B, C, D, E, H, I, J, ...}
   - 新面孔集合 N = T - S: [X, F, G, ...]
   - Step 7 描述对象（新面孔 ≤3 选 Top 5 优先）: X, F, G
   ```
6. **该备忘必须作为 Step 7 写 summary.md "今日新动向" 小节时的硬输入**——必须至少在"今日新动向"中写出一条具体的跨日对比事实（带 `owner/repo` 仓库名），不允许只写空话；且新面孔集合必须基于"近 3 天已上榜仓库集合 S"严格差集得到（T − S），禁止只看 Top 5 名次变化。
7. 如果 `<project_root>/github-trending-knowledge/` 目录不存在或没有任何历史归档（新项目首次运行），Step 0 退化为：通过脚本 pipeline 旁路记录今日 Top 5 即可，跳过对比，并在 Step 7 中注明"无历史归档可对比"。
...
2. **结构**：只写两个二级标题小节，**结构固定，不允许增加或减少**：
   ...
   - `### 今日新动向`：用 2-3 句话点出今日与昨日/近期相比新出现的变化、技术拐点或信号。**必须至少包含 1 条具体的跨日对比事实**（如"某某仓库连续 N 日 #1 后今日被 X 取代"或"新上榜 Y"），并使用具体仓库名（`owner/repo` 形式）——这是 Step 0 跨日对比机制的硬性产物，不允许只写"AI 全栈继续火热"等空话。
     - **新面孔一句话介绍**：对今日的"新面孔仓库"（即 Step 0 中严格定义的"近 3 天未上榜、今日首次上榜"集合 N 中的成员，**与名次无关**），应在该小节中用一句话点出"作用 + 差异化特征"，仓库名以 `owner/repo` 形式给出。该项与上方 "≤3 仓库举例" 配额正交：新面孔介绍属于点名亮相，不计入 ≤3 配额，但仍遵守"避免英文专名堆叠、意译优先"的语言风格。**两条细化要求**：(a) 最多只列举 3 个新面孔仓库，禁止把当日所有新面孔仓库一一介绍；(b) 当日多个新面孔仓库若属于同一方向/领域，应优先合并概括该方向，而不是逐个分别介绍。
3. **长度**：整篇 summary.md **不超过 300 字**(不含元信息行)。
4. **语言风格（言简意赅）**：
...
5. **仓库举例规则**：
   ...
   - 不允许在正文里以列表形式逐个介绍仓库。
   - 注：上述 "≤3 举例" 配额**不包含**"今日新动向"小节里对 Top 5 新面孔的一句话介绍；新面孔介绍属于点名亮相，与正文举例分别计数，且新面孔介绍本身也受"≤3 上限 + 同方向合并"约束。
